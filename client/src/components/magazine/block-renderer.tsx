 ...
 * Install the `dompurify` package and replace this with sanitize()
    ...
    <div className={`${getMaxWidth()} mx-auto ${className}`} data-testid="block-renderer">
      {sections.map((section) => {
        switch (section._type) {
          ...
          case "qa":
            return (
              <div key={section.id} className="my-10 space-y-8" data-testid={`qa-block-${section.id}`}>
                {(section as QABlock).questions.map((qa, i) => (
                  <div key={i} className="border-l-2 border-olive pl-6">
                    <p className="font-mono text-xs uppercase tracking-widest text-olive mb-2">{qa.question}</p>
                    <p className="font-serif text-lg leading-relaxed text-ink-muted">{qa.answer}</p>
                  </div>
                ))}
              </div>
            );
