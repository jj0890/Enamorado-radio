...
import {
  ...
  mixSubmissions as mixSubmissionsTable,
  currentPlayback as currentPlaybackTable,
  streamStatus as streamStatusTable,
  residents as residentsTable,
  tags as tagsTable,
...
import { registerGenreRoutes } from './genreRoutes';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/contributors/:handle', async (req, res) => {
    try {
      ...
      // Get editorial content via the contentContributors junction table
      let editorial: Array<{
        id: number;
        title: string;
        slug: string;
      ...
      try {
        // Query content linked to this contributor via junction table
        const contentCredits = await db
          .select({
            contentId: contentContributors.contentId,
            role: contentContributors.role,
            position: contentContributors.position,
            content: {
              id: content.id,
              title: content.title,
              slug: content.slug,
          ...
          .innerJoin(content, eq(contentContributors.contentId, content.id))
          .where(
            and(
              eq(contentContributors.contributorId, contributor.id),
              eq(content.status, 'published')
            )
          )
          .orderBy(contentContributors.position);

        editorial = contentCredits.map(cc => ({
          id: cc.content.id,
          title: cc.content.title,
          slug: cc.content.slug,
          excerpt: cc.content.excerpt,
          coverImageUrl: cc.content.coverImageUrl,
          contentType: cc.content.contentType,
          role: cc.role || 'author',
          publishedAt: cc.content.publishedAt,
        }));
      } catch (dbError) {
        // Junction table may not exist yet if migration hasn't run
      ...
      res.json({
        ...
        submissions: {
          ...
          playlists,
          editorial,
          total: mixes.length + playlists.length + editorial.length,
        }
      });
    } catch (error) {
  ...
  // Get contributors for a piece of content
  app.get('/api/content/:id/contributors', async (req, res) => {
    try {
      const contributors = await storage.getContentContributors(parseInt(req.params.id));
      res.json(contributors);
    } catch (error) {
      console.error('Error fetching content contributors:', error);
      res.status(500).json({ error: 'Failed to fetch content contributors' });
    }
  });

  // Atomically replace all contributors for a piece of content
  app.put('/api/content/:id/contributors', requireRole(['admin', 'editor']), async (req, res) => {
    try {
      const contentId = parseInt(req.params.id);
      const contributors = Array.isArray(req.body) ? req.body : [];
      const result = await storage.setContentContributors(
        contentId,
      ...
      broadcast({ type: 'content_contributors_updated', data: { contentId, contributors: result } });
      res.json(result);
    } catch (error) {
      console.error('Error updating content contributors:', error);
      res.status(500).json({ error: 'Failed to update content contributors' });
    }
  });
  ...
  app.post('/api/content', requireRole(['admin', 'editor']), async (req, res) => {
    try {
      ...
      if (Array.isArray(contributorsData) && contributorsData.length > 0) {
        await storage.setContentContributors(
          newContent.id,
          contributorsData.map((c: any, i: number) => ({
      ...
      broadcast({ type: 'content_created', data: newContent });
      res.json(newContent);
    } catch (error) {
  ...
  app.patch('/api/content/:id', requireRole(['admin', 'editor']), async (req, res) => {
    try {
      ...
      // If contributors array was explicitly included in the payload, sync it
      ...
      broadcast({ type: 'content_updated', data: updatedContent });
  ...
  // =================
  // GENRE & CONTENT ROUTES
  // =================
  registerGenreRoutes(app);

  // =================
  // EDITORIAL SUBMISSION ROUTES
  // =================
  registerSubmissionRoutes(app);

  // ============================================================
  // BEAM LAYER — /api/stream/*
  // Thin streaming endpoints — zero DB writes, status + history only
  // Inspired by Nina Protocol's beam.ninaprotocol.com pattern
  // ============================================================

  /** Health probe / polling heartbeat */
  app.get('/api/stream/ping', async (_req, res) => {
    try {
      const [status] = await db.select().from(streamStatusTable).limit(1);
      res.json({
        ok: true,
  ...
  /** Current track + stream status — single source of truth for "what's on now" */
  app.get('/api/stream/now', async (_req, res) => {
    try {
      ...
      res.json({
        ...
        currentShow: status?.currentShow ?? null,
        nowPlaying: playback
          ? {
              ...
              startedAt: playback.startTime,
              episodeId: playback.episodeId,
              mixId: playback.mixId,
            }
          : null,
      });
    } catch (error) {
      console.error('Error fetching /api/stream/now:', error);
      res.status(500).json({ error: 'Failed to fetch stream status' });
    }
  });

  /**
   * Recent playback history — used by "Recently Played" tickers
   * ?limit=N  max 20, default 7
   */
  app.get('/api/stream/recent', async (req, res) => {
    try {
      const limit = Math.min(parseInt(req.query.limit as string) || 7, 20);
      const plays = await db
        .select()
        .from(currentPlaybackTable)
        .orderBy(desc(currentPlaybackTable.startTime))
        .limit(limit);
      ...
      console.error('Error fetching /api/stream/recent:', error);
      res.status(500).json({ error: 'Failed to fetch recent plays' });
    }
  });

  // ============================================================
  // CONTENT API v1 — /api/v1/*
  // Versioned, paginated content surface — episodes + mixes + profiles + posts
  // ============================================================

  /**
   * Unified release feed (episodes + approved mixes)
   * ?type=episode|mix   filter by type (omit for both)
   * ?limit=N&offset=N   pagination (default 20, max 100)
   */
  app.get('/api/v1/releases', async (req, res) => {
    try {
      ...
      // Always run both queries; filter by typeFilter in JS — avoids union-type ambiguity
      const [allEps, allMixes] = await Promise.all([
        db.select().from(episodesTable)
          .where(eq(episodesTable.status, 'published'))
          .orderBy(desc(episodesTable.airDate))
          .limit(100),
        db.select().from(mixSubmissionsTable)
          .where(and(eq(mixSubmissionsTable.status, 'approved'), eq(mixSubmissionsTable.featureOnSite, true)))
          .orderBy(desc(mixSubmissionsTable.approved_at))
          .limit(100),
      ]);

      const episodeItems = (!typeFilter || typeFilter === 'episode')
        ? allEps.map(e => ({
            releaseId: `ep-${e.id}`,
            type: 'episode' as const,
            id: e.id,
            title: e.title,
            artist: e.hostName,
            artwork: e.artworkUrl,
            genre: e.genre,
            tags: e.tags ?? [],
            duration: e.duration,
            publishedAt: e.airDate,
            createdAt: e.createdAt,
          }))
        : [];

      const mixItems = (!typeFilter || typeFilter === 'mix')
        ? allMixes.map(m => ({
            releaseId: `mix-${m.id}`,
            type: 'mix' as const,
            id: m.id,
            ...
            artist: m.name,
            handle: m.handle,
            artwork: m.artUrl || m.artwork_url || m.coverUrl,
            genre: m.genre,
            publishedAt: m.approved_at,
            createdAt: m.createdAt,
          }))
        : [];

      const all = [...episodeItems, ...mixItems].sort((a, b) => {
      ...
      console.error('Error fetching /api/v1/releases:', error);
      res.status(500).json({ error: 'Failed to fetch releases' });
    }
  });

  /**
   * Single release by prefixed ID
   * :id format — "ep-{id}" for episodes, "mix-{id}" for mixes
   */
  app.get('/api/v1/releases/:id', async (req, res) => {
    try {
      const releaseId = req.params.id;

      if (releaseId.startsWith('ep-')) {
        const id = parseInt(releaseId.slice(3));
        if (isNaN(id)) return res.status(400).json({ error: 'Invalid episode ID' });
        const [ep] = await db.select().from(episodesTable).where(eq(episodesTable.id, id));
        if (!ep) return res.status(404).json({ error: 'Episode not found' });
        return res.json({ releaseId, type: 'episode', ...ep });
      }

      if (releaseId.startsWith('mix-')) {
        const id = parseInt(releaseId.slice(4));
        if (isNaN(id)) return res.status(400).json({ error: 'Invalid mix ID' });
        const [mix] = await db.select().from(mixSubmissionsTable).where(eq(mixSubmissionsTable.id, id));
        if (!mix) return res.status(404).json({ error: 'Mix not found' });
        return res.json({ releaseId, type: 'mix', ...mix });
      }

      res.status(400).json({ error: 'Invalid release ID format — use ep-{id} or mix-{id}' });
    } catch (error) {
      console.error('Error fetching /api/v1/releases/:id:', error);
      res.status(500).json({ error: 'Failed to fetch release' });
    }
  });

  /**
   * Active resident profiles
   * ?limit=N&offset=N  pagination (default 20, max 100)
   */
  app.get('/api/v1/profiles', async (req, res) => {
    try {
      ...
      const profiles = await db.select({
        ...
        handle: residentsTable.username,
        displayName: residentsTable.displayName,
        bio: residentsTable.bio,
        avatar: residentsTable.avatarUrl,
        showTitle: residentsTable.showTitle,
        showDescription: residentsTable.showDescription,
        genres: residentsTable.genres,
        socialLinks: residentsTable.socialLinks,
        createdAt: residentsTable.createdAt,
      })
        .from(residentsTable)
        .where(eq(residentsTable.isActive, true))
        .orderBy(asc(residentsTable.displayName))
        .limit(limit)
        .offset(offset);

      res.json({ total: profiles.length, items: profiles });
    } catch (error) {
      console.error('Error fetching /api/v1/profiles:', error);
      res.status(500).json({ error: 'Failed to fetch profiles' });
    }
  });

  /**
   * Single profile by handle (username) with recent releases
   */
  app.get('/api/v1/profiles/:handle', async (req, res) => {
    try {
      ...
      const [profile] = await db.select({
        ...
        .from(residentsTable)
        .where(and(eq(residentsTable.username, handle), eq(residentsTable.isActive, true)));

      if (!profile) return res.status(404).json({ error: 'Profile not found' });

      // Fetch last 5 published episodes hosted by this resident (name-matched)
      const recentEpisodes = await db.select()
        .from(episodesTable)
        .where(and(eq(episodesTable.hostName, profile.displayName), eq(episodesTable.status, 'published')))
        .orderBy(desc(episodesTable.airDate))
        .limit(5);

      res.json({
        ...profile,
        releases: recentEpisodes.map(e => ({ releaseId: `ep-${e.id}`, type: 'episode', ...e })),
      });
    } catch (error) {
      console.error('Error fetching /api/v1/profiles/:handle:', error);
   ...
   * ?type=essay|interview|photo_essay|playlist|video  filter by contentType
   * ?limit=N&offset=N  pagination (default 20, max 100)
   */
  app.get('/api/v1/posts', async (req, res) => {
    try {
      ...
      const posts = await db.select({
        ...
        title: content.title,
        excerpt: content.excerpt,
        coverImage: content.coverImageUrl,
        contentType: content.contentType,
      ...
      console.error('Error fetching /api/v1/posts:', error);
      res.status(500).json({ error: 'Failed to fetch posts' });
    }
  });

  // ============================================================
  // SERVICES / DISCOVERY — /api/v1/tags, /api/v1/hubs, /api/v1/discovery
  // Temporal tag ranking + randomized editorial surfacing + iceberg discovery
  // Inspired by Nina Protocol's services.ninaprotocol.com/v1 pattern
  // ============================================================

  /**
   * All public tags with per-tag content count
   */
  app.get('/api/v1/tags', async (_req, res) => {
    try {
      const allTags = await db.select({
        name: tagsTable.name,
        type: tagsTable.type,
        contentCount: sql<number>`count(${contentTagsTable.contentId})::int`,
      })
        .from(tagsTable)
        .leftJoin(contentTagsTable, eq(contentTagsTable.tagId, tagsTable.id))
        .where(eq(tagsTable.isPublic, true))
        .groupBy(tagsTable.id, tagsTable.name, tagsTable.type)
        .orderBy(desc(sql`count(${contentTagsTable.contentId})`));

      res.json(allTags);
    } catch (error) {
      console.error('Error fetching /api/v1/tags:', error);
      res.status(500).json({ error: 'Failed to fetch tags' });
    }
  });

  /**
   * Temporal tag discovery — Nina's key pattern
   * Returns editorial posts + episodes tagged with :tag, filtered by daterange
   *
   * ?daterange=weekly|monthly|alltime  (default alltime)
   * ?limit=N  (default 20, max 100)
   */
  app.get('/api/v1/tags/:tag', async (req, res) => {
    try {
      const tagName = req.params.tag;
      const daterange = (req.query.daterange as string) || 'alltime';
      const limit = Math.min(parseInt(req.query.limit as string) || 20, 100);

      // Resolve date cutoff
      let dateCutoff: Date | null = null;
      if (daterange === 'weekly') dateCutoff = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      else if (daterange === 'monthly') dateCutoff = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);

      // Look up the tag record
      const [tag] = await db.select().from(tagsTable).where(eq(tagsTable.name, tagName));
      if (!tag) return res.json({ tag: tagName, daterange, total: 0, items: [] });

      // Editorial content with this tag (via contentTags junction)
      const contentConditions: any[] = [eq(contentTagsTable.tagId, tag.id), eq(content.status, 'published')];
      if (dateCutoff) contentConditions.push(gt(content.publishedAt, dateCutoff));

      const taggedContent = await db.select({
        id: content.id,
        itemType: sql<string>`'post'`,
        title: content.title,
        slug: content.slug,
        excerpt: content.excerpt,
        coverImage: content.coverImageUrl,
        ...
        .from(content)
        .innerJoin(contentTagsTable, eq(contentTagsTable.contentId, content.id))
        .where(and(...contentConditions))
        .orderBy(desc(content.publishedAt))
        .limit(limit);

      // Episodes that list this tag in their tags text[] array
      const episodeConditions: any[] = [
        eq(episodesTable.status, 'published'),
        sql`${episodesTable.tags} @> ARRAY[${tagName}]::text[]`,
      ];
      if (dateCutoff) episodeConditions.push(gt(episodesTable.airDate, dateCutoff));

      const taggedEpisodes = await db.select({
        id: episodesTable.id,
        itemType: sql<string>`'episode'`,
        title: episodesTable.title,
        artist: episodesTable.hostName,
        artwork: episodesTable.artworkUrl,
        genre: episodesTable.genre,
        publishedAt: episodesTable.airDate,
      })
        .from(episodesTable)
      ...
      const allItems = [...taggedContent, ...taggedEpisodes].sort((a, b) => {
        return (b.publishedAt ? new Date(b.publishedAt as any).getTime() : 0)
             - (a.publishedAt ? new Date(a.publishedAt as any).getTime() : 0);
      });

      res.json({ tag: tagName, daterange, total: allItems.length, items: allItems.slice(0, limit) });
    } catch (error) {
      console.error('Error fetching /api/v1/tags/:tag:', error);
      res.status(500).json({ error: 'Failed to fetch tag content' });
    }
  });

  /**
   * All active editorial feature hubs
   */
  app.get('/api/v1/hubs', async (_req, res) => {
    try {
      const hubs = await db.select({
        id: featuresTable.id,
        entityType: featuresTable.entityType,
        entityId: featuresTable.entityId,
        featureType: featuresTable.featureType,
        position: featuresTable.position,
        activeFrom: featuresTable.activeFrom,
        activeUntil: featuresTable.activeUntil,
      })
        .from(featuresTable)
      ...
      console.error('Error fetching /api/v1/hubs:', error);
      res.status(500).json({ error: 'Failed to fetch hubs' });
    }
  });

  /**
   * Hub content by featureType — with optional randomized surfacing
   * Nina uses ORDER BY RANDOM() on "staff-picks" to surface non-chronological gems
   *
   * :featureType — e.g. "hero", "main", "secondary"
   * ?random=true   shuffle results (default false)
   * ?limit=N       (default 15, max 50)
   */
  app.get('/api/v1/hubs/:featureType', async (req, res) => {
    try {
      ...
      const baseQuery = db.select()
        .from(featuresTable)
        .where(and(eq(featuresTable.isActive, true), eq(featuresTable.featureType, featureType)));

      const hubItems = await (isRandom
        ? baseQuery.orderBy(sql`RANDOM()`).limit(limit)
        : baseQuery.orderBy(asc(featuresTable.position)).limit(limit));

      // Resolve entity details inline
      const resolvedItems = await Promise.all(
        hubItems.map(async item => {
          try {
            const id = parseInt(item.entityId);
            if (isNaN(id)) return { ...item, resolved: null };

            if (item.entityType === 'content') {
              const [c] = await db.select({
                id: content.id, title: content.title, slug: content.slug,
                excerpt: content.excerpt, coverImage: content.coverImageUrl,
                publishedAt: content.publishedAt,
              }).from(content).where(eq(content.id, id));
              return { ...item, resolved: c ?? null };
            }

            if (item.entityType === 'episode') {
              const [e] = await db.select({
                id: episodesTable.id, title: episodesTable.title,
                artwork: episodesTable.artworkUrl, artist: episodesTable.hostName,
                publishedAt: episodesTable.airDate,
              }).from(episodesTable).where(eq(episodesTable.id, id));
              return { ...item, resolved: e ?? null };
            }

            if (item.entityType === 'submission') {
              const [s] = await db.select({
                id: submissionsTable.id, title: submissionsTable.title,
                excerpt: submissionsTable.description,
                coverImage: submissionsTable.coverImageUrl,
                submitterHandle: submissionsTable.submitterHandle,
                category: submissionsTable.category,
              }).from(submissionsTable).where(eq(submissionsTable.id, id));
              return { ...item, resolved: s ?? null };
            }

            if (item.entityType === 'mix') {
              const [m] = await db.select({
                id: mixSubmissionsTable.id, title: mixSubmissionsTable.title,
                artist: mixSubmissionsTable.name, artwork: mixSubmissionsTable.artwork_url,
                genre: mixSubmissionsTable.genre,
              }).from(mixSubmissionsTable).where(eq(mixSubmissionsTable.id, id));
              return { ...item, resolved: m ?? null };
            }
      ...
      res.json({ featureType, random: isRandom, total: resolvedItems.length, items: resolvedItems });
    } catch (error) {
      console.error('Error fetching /api/v1/hubs/:featureType:', error);
      res.status(500).json({ error: 'Failed to fetch hub content' });
    }
  });

  /**
   * Iceberg discovery — hidden gems: published 14–90 days ago with low view count
   * Falls back to a date-window random sample if not enough low-play content exists
   *
   * ?limit=N  (default 10, max 30)
   */
  app.get('/api/v1/discovery/iceberg', async (req, res) => {
    try {
      ...
      // Primary: low viewCount (≤50) in the 14–90 day window
      const hiddenGems = await db.select({
        releaseId: sql<string>`'ep-' || ${episodesTable.id}::text`,
        ...
        .where(
          and(
            eq(episodesTable.status, 'published'),
            gt(episodesTable.airDate, ninetyDaysAgo),
            lt(episodesTable.airDate, fourteenDaysAgo),
            sql`${episodesTable.viewCount} <= 50`,
          )
        )
        .orderBy(desc(episodesTable.airDate))
        .limit(limit);
      ...
      if (hiddenGems.length >= 3) {
        return res.json({ strategy: 'low-plays-date-window', total: hiddenGems.length, items: hiddenGems });
      }

      // Fallback: random sample from the same date window (no viewCount filter)
      ...
      res.json({ strategy: 'date-window-sample', total: fallback.length, items: fallback });
    } catch (error) {
      console.error('Error fetching /api/v1/discovery/iceberg:', error);
      res.status(500).json({ error: 'Failed to fetch discovery content' });
    }
  });

  // ============================================================
  // LISTENER LIKES + COMMUNITY PROMOTION
  // Anonymous session-based likes on submissions, episodes, mixes.
  // Auto-promotes to editorial_features once threshold is crossed.
  // ============================================================

  const LIKE_COOKIE = 'enamorado_lsid';
  const VALID_LIKE_ENTITY_TYPES = ['submission', 'episode', 'mix', 'content'] as const;
  type LikeEntityType = typeof VALID_LIKE_ENTITY_TYPES[number];

  function getOrCreateSessionKey(req: any, res: any): string {
    let key = req.cookies?.[LIKE_COOKIE];
    if (!key) {
      key = crypto.randomUUID();
      res.cookie(LIKE_COOKIE, key, {
        httpOnly: true,
  ...
  /** Check like count for entity; auto-promote if threshold crossed */
  async function checkAndPromote(entityType: LikeEntityType, entityId: number): Promise<{ count: number; promoted: boolean }> {
    const [{ value: likeCount }] = await db
      .select({ value: count() })
      .from(contentLikesTable)
      .where(and(eq(contentLikesTable.entityType, entityType), eq(contentLikesTable.entityId, entityId)));

    const total = Number(likeCount ?? 0);

    // Read threshold from settings (default 10)
    const [setting] = await db.select().from(settingsTable).where(eq(settingsTable.key, 'like_promotion_threshold'));
    const threshold = parseInt(setting?.value ?? '10');

    if (total < threshold) return { count: total, promoted: false };

    // Check if community feature slot already exists
    const [existing] = await db.select().from(featuresTable).where(
      and(
        eq(featuresTable.entityType, entityType),
        eq(featuresTable.entityId, String(entityId)),
        eq(featuresTable.featureType, 'community'),
      )
    );
    if (existing) return { count: total, promoted: false };
    ...
    return { count: total, promoted: true };
  }

  /**
   * Toggle like on a piece of content (idempotent)
   * POST /api/likes/:entityType/:id
   * entityType: submission | episode | mix
   * Returns: { liked: boolean, count: number, promoted?: boolean }
   */
  app.post('/api/likes/:entityType/:id', async (req, res) => {
    try {
      const entityType = req.params.entityType as LikeEntityType;
      const entityId = parseInt(req.params.id);

      if (!VALID_LIKE_ENTITY_TYPES.includes(entityType) || isNaN(entityId)) {
        return res.status(400).json({ error: 'Invalid entityType or id. Use submission | episode | mix' });
      }

      const sessionKey = getOrCreateSessionKey(req, res);

      // Check for existing like
      const [existing] = await db.select().from(contentLikesTable).where(
        and(
          eq(contentLikesTable.entityType, entityType),
          eq(contentLikesTable.entityId, entityId),
      ...
      if (existing) {
        ...
        const [{ value: newCount }] = await db
          ...
          .from(contentLikesTable)
          .where(and(eq(contentLikesTable.entityType, entityType), eq(contentLikesTable.entityId, entityId)));
        return res.json({ liked: false, count: Number(newCount ?? 0) });
      }

      // Like — insert new row
      await db.insert(contentLikesTable).values({ entityType, entityId, sessionKey });

      // Check auto-promotion
      const { count: total, promoted } = await checkAndPromote(entityType, entityId);
      res.json({ liked: true, count: total, ...(promoted && { promoted: true }) });
    } catch (error) {
  ...
  /**
   * Get like status for current session + total count
   * GET /api/likes/:entityType/:id
   * Returns: { liked: boolean, count: number }
   */
  app.get('/api/likes/:entityType/:id', async (req, res) => {
    try {
      const entityType = req.params.entityType as LikeEntityType;
      const entityId = parseInt(req.params.id);

      if (!VALID_LIKE_ENTITY_TYPES.includes(entityType) || isNaN(entityId)) {
        return res.status(400).json({ error: 'Invalid entityType or id' });
      }

      const sessionKey = req.cookies?.[LIKE_COOKIE];
      ...
      res.json({ liked, count: Number(total ?? 0) });
    } catch (error) {
  ...
  /**
   * Trending content by listener like velocity
   * GET /api/v1/discovery/trending?daterange=weekly|monthly&limit=10
   * Groups likes by entity, filtered by recency, resolves entity details
   */
  app.get('/api/v1/discovery/trending', async (req, res) => {
    try {
      const daterange = (req.query.daterange as string) || 'weekly';
      const limit = Math.min(parseInt(req.query.limit as string) || 10, 50);

      const cutoff = daterange === 'monthly'
        ? new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
        : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);

      // Aggregate likes within the window, ranked by count desc
      const ranked = await db
        .select({
          entityType: contentLikesTable.entityType,
          entityId: contentLikesTable.entityId,
          likeCount: sql<number>`count(*)::int`,
        })
        .from(contentLikesTable)
        .where(gt(contentLikesTable.createdAt, cutoff))
        .groupBy(contentLikesTable.entityType, contentLikesTable.entityId)
        ...
        ranked.map(async row => {
          try {
            ...
            if (row.entityType === 'submission') {
              ...
              return { ...row, resolved: s ?? null };
            }
            if (row.entityType === 'episode') {
              const [e] = await db.select({
                id: episodesTable.id, title: episodesTable.title,
                artist: episodesTable.hostName, artwork: episodesTable.artworkUrl,
                genre: episodesTable.genre, publishedAt: episodesTable.airDate,
              }).from(episodesTable).where(eq(episodesTable.id, id));
              return { ...row, resolved: e ?? null };
            }
            if (row.entityType === 'mix') {
              const [m] = await db.select({
                id: mixSubmissionsTable.id, title: mixSubmissionsTable.title,
                artist: mixSubmissionsTable.name, artwork: mixSubmissionsTable.artwork_url,
                genre: mixSubmissionsTable.genre,
              }).from(mixSubmissionsTable).where(eq(mixSubmissionsTable.id, id));
              return { ...row, resolved: m ?? null };
            }
      ...
      res.json({ daterange, total: items.length, items });
    } catch (error) {
  ...
  // ── Extend /api/v1/hubs/:featureType to resolve 'submission' and 'mix' entities ─
  // The existing handler (above) only resolves 'content' and 'episode'.
  // Community-promoted items use entityType 'submission' and 'mix', so we patch
  // the resolver by adding a dedicated community hub endpoint that handles all types.

  /**
   * Community hub — community-promoted content with full resolution
   * GET /api/v1/hubs/community/resolved
   * A superset of /api/v1/hubs/community that resolves submission + mix entity types too
   */
  app.get('/api/v1/hubs/community/resolved', async (req, res) => {
    try {
      const isRandom = req.query.random === 'true';
      const limit = Math.min(parseInt(req.query.limit as string) || 15, 50);
      ...
      const hubItems = await (isRandom
        ? db.select().from(featuresTable)
            .where(and(eq(featuresTable.isActive, true), eq(featuresTable.featureType, 'community')))
            .orderBy(sql`RANDOM()`).limit(limit)
        : db.select().from(featuresTable)
            .where(and(eq(featuresTable.isActive, true), eq(featuresTable.featureType, 'community')))
            .orderBy(asc(featuresTable.position)).limit(limit));
        ...
        hubItems.map(async item => {
          try {
            ...
            if (item.entityType === 'submission') {
              ...
              // Attach like count
              const [{ value: likeCount }] = await db.select({ value: count() })
                .from(contentLikesTable)
                .where(and(eq(contentLikesTable.entityType, 'submission'), eq(contentLikesTable.entityId, id)));
              return { ...item, resolved: s ?? null, likeCount: Number(likeCount ?? 0) };
            }
            ...
            if (item.entityType === 'episode') {
              ...
              const [{ value: likeCount }] = await db.select({ value: count() })
                .from(contentLikesTable)
                .where(and(eq(contentLikesTable.entityType, 'episode'), eq(contentLikesTable.entityId, id)));
              return { ...item, resolved: e ?? null, likeCount: Number(likeCount ?? 0) };
            }
            ...
            if (item.entityType === 'mix') {
              ...
              const [{ value: likeCount }] = await db.select({ value: count() })
                .from(contentLikesTable)
                .where(and(eq(contentLikesTable.entityType, 'mix'), eq(contentLikesTable.entityId, id)));
              return { ...item, resolved: m ?? null, likeCount: Number(likeCount ?? 0) };
            }

            return { ...item, resolved: null, likeCount: 0 };
          } catch {
            return { ...item, resolved: null, likeCount: 0 };
          }
        })
      ...
      res.json({ featureType: 'community', random: isRandom, total: resolvedItems.length, items: resolvedItems });
    } catch (error) {
